...
\begin{figure}[H]
   \caption{Schematic flowchart of the eABF algorithm}
        ...
        \node [block, below of=la_hist] (R) {$k_\lambda = f(k_\lambda)$ (eq. \ref{eq:ramp})};
        \node [block, below of=R] (F_B1) {$F_B(k_\lambda)\pluseqq \frac{1}{\sigma^2}(\lambda-\xi)$};
        \node [block, below of=F_B1] (F_B2) {$F(\textbf{x},\lambda) \pluseqq \frac{F_B(k_\lambda)}{\tilde{\rho}_\lambda(k_\lambda)}\nabla\lambda$};
        ...
        \node [block, below of=decide_xi,xshift=-3cm] (xi_bin) {$k_\lambda = \lfloor \frac{\xi - \xi_{min}}{\Delta \xi} \rfloor$};
        ...
        \node [block,below of=F_corr,xshift=3cm] (MD1) {Finish Langevin dynamics of extended system};

        \node [decision, below of=MD1,xshift=3cm] (output) {$t=t_{out}$};
        \node [block, left of=output,xshift=-4cm] (CZAR) {calculate CZAR according to eq. \ref{eq:CZAR}};
        \node [block, left of=CZAR,xshift=-4cm] (out) {write output};

        \node [block, left of=R,xshift=-4cm] (MD2) {MD step of system according to Langevin dynamics};

        \path [line] (xi) -- (langevin);
        \path [line] (langevin) -- (F_ext);
        \path [line] (F_ext) -- (decide_lambda);

        \draw [->] (decide_lambda) -- (la_bin);
        \path [line] (la_bin) -- (la_hist);
        \path [line] (la_hist) -- (R);
        \path [line] (R) -- (F_B1);
        \path [line] (F_B1) -- (F_B2);

        \draw [->] (decide_lambda) -- (conf1);

        \path [line] (F_B2) -- (decide_xi);
        \path [line] (conf1) -- (decide_xi);

        \draw [->] (decide_xi) -- (xi_bin);
        \path [line] (xi_bin) -- (xi_hist);
        \path [line] (xi_hist) -- (F_corr);

        \draw [->] (decide_xi) -- (conf2);

        \path [line] (F_corr) -- (MD1);
        \path [line] (conf2) -- (MD1);

        \path [line] (MD1) -- (output);
        \path [line] (output) -- (CZAR);
        \path [line] (CZAR) -- (out);
