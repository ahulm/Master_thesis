...
\begin{figure}[H]
    \centering
    \subfloat[Trajectories of $\xi$ and $\lambda$, \\
    Inset: Histogram of ($\lambda-\xi$) with standard deviation $\sigma$]{{\includegraphics[width=0.49\textwidth]{bilder/eABF_traj} }}
    \subfloat[Biased $\xi$- or $\lambda$-conditioned probability distributions $\tilde{p}_\xi$ and $\tilde{p}_\lambda$]{{\includegraphics[width=0.49\textwidth]{bilder/eABF_prob} }}
...
\begin{figure}[H]
   \caption{
     Flowchart of the eABF algorithm. $k_{\lambda,\xi}$ denote $\lambda$ or $\xi$ conditioned bin indices in bins with size $\Delta\xi$. $\tilde{\rho}_\lambda$ and $\tilde{\rho}_\xi$ are $\lambda$- or $\xi$- conditioned histograms and $R$ is the ABF ramp function. $F_B$ is the sum bias force samples. Outputs are written in fixed intervals.
        ...
        \node [decision, below of=MD1,xshift=3cm] (output) {write output?};
        ...
        \node [block, left of=R,xshift=-4cm] (MD2) {MD step of physical system. (compare Algorithm \ref{alg:ABM})};
        ...
        \path [line] (output) -- node [above] {yes} (CZAR);
        \path [line] (CZAR) -- (out);
