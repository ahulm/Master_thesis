...
def metaD(self, ats, variance=1.0, height=0.1, time_int=20, WT=True, dT=20):
    ...
    if self.step == 0: 
        ...
        self.mean_grad  = np.array(np.zeros(self.nbins), dtype=np.float64)
    ...
    if (xi <= self.maxx).all() and (xi >= self.minx).all():
        ...
        self.mean_grad[bink] += np.linalg.norm(delta_xi)
        ...
        for i in range(len(ats)):
            ...
            if self.step%time_int == 0:
                ...
                dx = self.grid - xi[i]
                bias_factor = self.height * np.exp(-0.5*np.power(dx,2.0)/variance)
    ...
    if self.step%10000 == 0:
        ...
        out.write("%6s\t%14s\t%14s\t%14s\t%14s\n" % ("Bin", "Xi", "Bias Pot", "Bias Force", "mean grad"))
        for i in range(len(self.bias_pot[0])):
            out.write("%6d\t%14.6f\t%14.6f\t%14.6f\t%14.6f\n" % (i, self.grid[i], self.bias_pot[0][i], self.bias_force[0][i], self.mean_grad[i]/self.bin_list[i] if self.bin_list[i] > 0 else 0))
        out.close()
